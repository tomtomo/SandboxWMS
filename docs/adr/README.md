...
| [0034](0034-allocation-failure-signaling.md) | Allocation-shortfall signaling (stock short) | Accepted | Event eksplisit `inventory.stock_allocation_shortfall.v1` saat wave allocation tak terpenuhi (ganti silent-drop); Outbound tandai `OrderLine` Short/Backordered + Notification alert; pola eShop `OrderStockRejected`, bukan sync ATP-gate; katalog 7→8, dijaga [ADR-0023](0023-event-contract-catalog-asyncapi.md). *(Amendment 2026-06-25: rename `Failed`→`Shortfall` via [ADR-0035](0035-auto-cancel-unfulfillable-wave.md))* |
| [0035](0035-auto-cancel-unfulfillable-wave.md) | Auto-cancel unfulfillable wave + return orders to backlog | Accepted | Wave nol-terpenuhi (`StockAllocated` kosong) → auto-cancel + order `InProgress→New` (re-waveable, **bukan** refuse customer); event-driven intra-Outbound, nol scheduler; `WaveStatus.Cancelled` baru (tanpa migrasi schema); pertahankan **2 event** (vs 1 gabungan); berlandas [ADR-0034](0034-allocation-failure-signaling.md) |
...
0034 allocation-shortfall signaling ── compensating-signal event ──▶ Outbound line Short/Backorder + Notification; ganti silent-drop overview §B#2; berlandas 0005·0010·0017, pola 0028·0030, dijaga 0023(FF#11)
0035 auto-cancel unfulfillable wave ── local compensating action ──▶ wave Cancelled + order→backlog saat StockAllocated kosong; cegah hang; event-driven intra-Outbound; berlandas 0034·0010·0031
